...
class LeaveTable():
    def __init__(self):
        ...
        self.x_max = float(rospy.get_param("~x_max", 0.5))
        ...
        # Publishers
        self.clusters_pub = rospy.Publisher("filtered_pc2", PointCloud2, queue_size=1)
...
#            print "leave table - service cancelled"
    ...
    def pc2CB(self, msg):
        if self.init:
            ...
            fil_cloud = filtering_helper.do_passthrough_filter(fil_cloud, 'intensity', 300, 4096)
...
#            print "table_pathpts: ", len(table_pathpts)
            ...
            if(len(table_pathpts)==2):
                ...
                path_pts.append(self.getPointOnLine(m1, 0.8, table_pathpts[1], self.start_pt))
                ...
                self.vizPoint(2, path_pts[2], ColorRGBA(1,0,1,1), "map") # Magenta
#                print len(path_pts)
