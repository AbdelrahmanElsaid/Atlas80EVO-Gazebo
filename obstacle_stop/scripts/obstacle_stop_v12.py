...
class ObstacleStopV8():
    ...
    def scanCB(self, msg):
        ...
        for i in xrange(len(msg.ranges)):
            ...
            if (-2.0<pt.x<2.0 and -1.0<pt.y<1.0):
                if self.lifter_status=="bottom":
                    if(self.mode!="table_dropping" and self.mode!="table_picking" and self.outer_zone.contains(shapely_point)):
                        overall_pts.append(pt)
                    if(self.mode=="table_dropping" and self.mode=="table_picking" and self.inner_zone.contains(shapely_point)):
                     #inner_pts.appnd(pt)
                        overall_pts.append(pt)

                if self.lifter_status=="top":
                    if (self.speed_type==1 and self.inner_region_top_speed_zone.contains(shapely_point)):
                        overall_pts.append(pt)
                    if(self.speed_type==0 and self.inner_region_slow_speed_zone.contains(shapely_point)):
        ...
        if self.data!="ERROR":
            self.err=1
        if (front_points!=[]) and (self.mode=="delivery" or self.mode=="error"):
