...
class ObstacleStopV2_2():
    ...
    def pc2CB(self, msg):
        ...
        clusters = self.getClusters(fil_cloud, 0.05, 5, 10000)
...
#        print "middle_time_2", self.middle_time_2
    ...
    def getDetectionRegion(self, fsm_state, lifter_status, speed):
        ...
        if(fsm_state=="DELIVERY" and lifter_status=="bottom"):
            ...
            detection_region = self.empty_region
        if(diff_spd >= 0.2 and fsm_state!="ERROR" and fsm_state!="SUSPEND"):
...
#        new_y = [(y + diff_spd) if y==max(orig_y) else (y - diff_spd) if y==min(orig_y) else y for y in orig_y]
        new_region = [list(a) for a in zip(new_x, orig_y)]
...
if __name__=="__main__":
    rospy.init_node("obstacle_stop_v2_2")
    ObstacleStopV2_2()
    rospy.spin()
