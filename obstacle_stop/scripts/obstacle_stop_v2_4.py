...
class ObstacleStopV2_4():
    def __init__(self):
        ...
        self.snapshot_call = rospy.ServiceProxy("/snapshot", Empty)
...
#        self.load_params_from_yaml(self.filepath)
        ...
        detected_pts = self.getPointsInRegionFromClusters(clusters, filtered_cloud, detection_region)
...
#            if(len(detected_pts)!=0 and self.fsm_state=="SUSPEND" and self.error==1):
            if(len(detected_pts)!=0 and self.fsm_state=="SUSPEND" and self.fsm_state!="ESTOP" and self.error==1):
                self.error = 0
                print "######################################"
                print "##########     snapshot     ##########"
                print "######################################"
...
#        elif(fsm_state=="ERROR" or fsm_state=="SUSPEND"):
        elif(fsm_state=="ERROR" or fsm_state=="SUSPEND" or fsm_state=="ESTOP"):
            detection_region = self.prev_detection_region
        else:
            detection_region = self.empty_region
        if(diff_spd >= 0.05 and fsm_state!="ERROR" and fsm_state!="SUSPEND"):
    ...
    def getRegionExtended(self, region, diff_spd):
        ...
        new_x = [(x + diff_spd + 0.2) if x==max(orig_x) else x for x in orig_x]
...
if __name__=="__main__":
    rospy.init_node("obstacle_stop_v2_4")
    ObstacleStopV2_4()
    rospy.spin()
