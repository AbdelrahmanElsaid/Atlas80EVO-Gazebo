...
#            self.route_pub.publish(Path())
    ...
    def pc2CB(self, msg):
        if self.init:
            ...
            clusters = self.getClusters(colorless_cloud, 0.05, 3, 350)
            ...
            if(len(mean_pts)>=2):
                for p1 in mean_pts:
                    for p2 in mean_pts:
                        ...
                        if(0.5 < self.getDistance(p1, p2) < 0.6):
                ...
                # Insert another point on the normal line with d[m] away from middle_pt
                path_pts.insert(-1, self.getPointOnNormalLine(m1, 1.0, middle_pt, self.start_pt))
