...
class HealthMonitoring():
	...
	# variables for safe function " no error detected " ---- # no mkore use !!!
...
#		if self.fsm_state == "ERROR" or msg.state== "MANUAL":
#			print ' fsm is already in error state or manual state'
#			self.call_error_state = False
#
#		if self.fsm_state !="ERROR" and msg.state!="MANUAL":
#			self.call_error_state = True
#			print 'fsm not in error state nor manual mode '
...
#		if self.str_error != 0 and self.call_error_state == True:
...
#
	...
	def main_loop(self):
		while not rospy.is_shutdown():
			...
			if (self.estop_check == True):
				self.estop_timestamp()
			if (self.bumper_check == True):
				self.bumper_timestamp()
			...
			if((rospy.Time.now().to_sec()-self.obs_secnow)>5):
				self.obstacle_safe = True
				self.error_arr [8] = 0
                                self.obs_secnow=rospy.Time.now().to_sec()
					# print "obs safe= True"
