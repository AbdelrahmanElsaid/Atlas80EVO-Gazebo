...
# flages for locking the functions for debgging matter 
	...
	def restore_response (self, request):
		...
		print '----------------------------------------------'
		print 'restore Service required', self.restore_counter
		print '----------------------------------------------'
		self.restore_triggerd = True
		return ()
	...
	def safe_check(self):
		...
		if self.str_error != 0:
			self.STOP()
			if (self.fsm_state!="MANUAL" and self.fsm_state!="ERROR"):
			...
			if (self.obstacle_check == True):
				if (self.obstacle_triggerd == True):
					...
					if(self.obs_diff.to_sec() < 5):
						self.obs_sens = int(rospy.get_param("~obs_sens",2))
