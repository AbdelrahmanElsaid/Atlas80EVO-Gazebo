...
#                        update_deli_dt=requests.get("http://13.251.126.12/at80_web/real_app/api/agv_deliupdate.php?id={}&status=COMPLETED".format(self.to_web_Delivery_ID))
                    ...
                    if self.to_web_Delivery_Status=="COMPLETED" or self.to_web_Delivery_Status=="CANCELLED" or self.to_web_Delivery_Status=="INTERRUPTED":
                        update_deli_dt=requests.get("http://13.251.126.12/at80_web/real_app/api/agv_deliupdate.php?id={}&status={}".format(self.to_web_Delivery_ID, self.to_web_Delivery_Status))
...
if __name__=="__main__":
    rospy.init_node("web_server_handler_v7")
    WebServerHandlerV7()
    rospy.spin()
