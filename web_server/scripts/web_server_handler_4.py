...
class WebServerHandler():
    ...
    def __init__(self):
        ...
        with open('/home/arefeen/catkin_ws/src/web_server/scripts/atlasprofile.json', 'r') as f:
        ...
        self.api_dowm_srv=rospy.ServiceProxy("/api/status",Empty)
    ...
    def image_to_web(self,value):
        if value==1:
            try:
                ...
                img_response = requests.post('http://13.251.126.12:8001?query=upload',files=self.im)

            except:
                self.im="no image path is given"

            try:
                requests.get("http://13.251.126.12/at80_web/real_app/api/agv_inserterror.php?agv_id={}&err_id={}&img={}".format(self.to_web_Agv_ID, self.to_web_Error_ID, str(img_response.text)))
            except:
    ...
    def main_loop(self):
        LDT= (requests.get('http://13.251.126.12/at80_web/real_app/api/agv_lastdtls.php?agv_id={}'.format(self.to_web_Agv_ID)))
        ...
        if (LDT.status_code == 200):
            ...
            if self.to_web_Battery!="":
                ...
                if self.to_web_Delivery_Location != Last_Delivery_details['location'] or ((self.to_web_Delivery_Location == Last_Delivery_details['location']) and self.to_web_Delivery_Status!=Last_Delivery_details['mis_status']):
                    update_deli_info=requests.get("http://13.251.126.12:8001?table=deli_dtls&query=insert&val=deli_id={}, location={},agv_id={}, mis_status={},ms_name={},mis_activity={}"
