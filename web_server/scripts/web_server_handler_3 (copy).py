    ...
    def image_to_web(self,value):
        if value==1:
            try:
                ...
                response = requests.post('http://13.251.126.12:8001?query=upload',files=self.im)
            except:
                self.im="no image path is given"

            try:
                requests.get("http://13.251.126.12:8001?table=error_hist&query=insert&val=agv_id={},err_id={},img={}".format(self.to_web_Agv_ID, self.to_web_Error_ID, str(self.img_path)))
            except:
            ...
            try:
                requests.get("http://13.251.126.12:8001?table=error_hist&query=insert&val=agv_id={},err_id={}".format(self.to_web_Agv_ID, self.to_web_Error_ID))
...
#                    print(self.to_web_Agv_ID + "-" + self.to_web_Location_x + "-" +  self.to_web_Location_y + "-" +  self.to_web_Speed + "-" + self.to_web_Battery + "-" +  self.to_web_Vehicle_status + "-" + self.to_web_Delivery_Status + "-" +  self.to_web_Error_ID  + "-" +  self.to_web_Delivery_Location, + "-" +  self.to_web_Action  + "-" +  self.to_web_Delivery_ID  + "-" +  self.to_web_Delivery_Route)
                ...
                if self.to_web_Delivery_Location != Last_Delivery_details['location'] or ((self.to_web_Delivery_Location == Last_Delivery_details['location']) and self.to_web_Delivery_Status!=Last_Delivery_details['mis_status']):
                    if (self.to_web_Delivery_Location=="HOME" and self.to_web_Delivery_Status=="ARRIVED"):
                        ...
                        update_deli_info=requests.get("http://13.251.126.12:8001?table=deli_dtls&query=insert&val=deli_id={}, location={},agv_id={}, mis_status={},ms_name={},mis_activity={}".format(self.to_web_Delivery_ID, self.to_web_Delivery_Location,  self.to_web_Agv_ID, self.to_web_Delivery_Status, self.to_web_Delivery_Route, self.to_web_Action))
                    
                    if self.to_web_Delivery_Status=="COMPLETED":
                        update_deli_dt=requests.get("http://13.251.126.12:8001/?table=deli_dt&query=update&field=id={}&val=status=COMPLETED".format(self.to_web_Delivery_ID))
