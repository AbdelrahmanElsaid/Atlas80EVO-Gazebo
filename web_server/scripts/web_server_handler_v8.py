...
"""
Features: (1) changed url will automatically change the url in the main codes.
"""


class WebServerHandlerV8():
    def __init__(self):
#        self.url="http://13.251.126.12:8001?"      #uncomment to send the data in database (database url:port)
        self.url=rospy.get_param("~url", "http://13.251.126.12")      #uncomment to send the data in database (database url:port)
        ...
        with open('/home/brobotics00/atlas80evo/gazebo_ws/src/atlas80evo/config/atlasprofile.json', 'r') as f:
        ...
        # services server
        self.set_file_location=rospy.Service("/file_location/snapshot", SetFileLocation, self.image_from_ros)
    ...
    def image_to_web(self,value):
        if value==1:
            try:
                ...
                print(self.im)
#                response = requests.post(self.url + ":8000?query=upload",files=self.im)
                response = requests.post("http://atlas80-psa-prod.southeastasia.cloudapp.azure.com" + ":8000?query=upload",files=self.im) # testing
                print(response.url)
                print("pass the response")
                print(response.status_code)
                requests.get(self.url + "/at80_web/real_app/api/agv_inserterror.php?agv_id={}&err_id={}&img={}".format(self.to_web_Agv_ID, self.to_web_Error_ID, str(response.text)))
            ...
            try:
                requests.get(self.url + "/at80_web/real_app/api/agv_inserterror.php?agv_id={}&err_id={}&img=".format(self.to_web_Agv_ID, self.to_web_Error_ID))
                print("errorid:",self.to_web_Error_ID)
            except:
    ...
    def main_loop(self):
        try:
            self.LDT= (requests.get(self.url + "/at80_web/real_app/api/agv_lastdtls.php?agv_id={}".format(self.to_web_Agv_ID)))
...
#            try:
            ...
            Last_Health=(requests.get(self.url + "/at80_web/real_app/api/agv_lasthealth.php?agv_id={}".format(self.to_web_Agv_ID))).json()
...
#                self.to_web_Delivery_Status = self.agv_Delivery_Status
                ...
                # Cancelling delivery - Only compatible with navigation_master_v1_9
                if Last_Delivery_details['mis_status']=="CANCELLING" and self.cancel_count < 2:
                    self.cancel_call()
                    self.cancel_count += 1
...
#                    print(self.to_web_Agv_ID + "-" + self.to_web_Location_x + "-" +  self.to_web_Location_y + "-" +  self.to_web_Speed + "-" + self.to_web_Battery + "-" +  self.to_web_Vehicle_status + "-" + self.to_web_Delivery_Status + "-" +  self.to_web_Error_ID  + "-" +  self.to_web_Delivery_Location, + "-" +  self.to_web_Action  + "-" +  self.to_web_Delivery_ID  + "-" +  self.to_web_Delivery_Route)
                ...
                if (self.to_web_Delivery_Location != Last_Delivery_details['location']) or ((self.to_web_Delivery_Location == Last_Delivery_details['location']) and (self.to_web_Delivery_Status!=Last_Delivery_details['mis_status'])):
                    if (self.to_web_Delivery_Location=="HOME" and self.to_web_Delivery_Status=="ARRIVED"):
                        ...
                        update_deli_info=requests.get(self.url + "/at80_web/real_app/api/agv_insertdtls.php?deli_id={}&location={}&agv_id={}&mis_status={}&ms_name={}&mis_activity={}".format(self.to_web_Delivery_ID, self.to_web_Delivery_Location,  self.to_web_Agv_ID, self.to_web_Delivery_Status, self.to_web_Delivery_Route, self.to_web_Action))

#                    if self.to_web_Delivery_Status=="COMPLETED":
#                        update_deli_dt=requests.get(self.url + "/at80_web/real_app/api/agv_deliupdate.php?id={}&status=COMPLETED".format(self.to_web_Delivery_ID))
                    # Only compatible with navigation_master_v1_9
                    if self.to_web_Delivery_Status=="COMPLETED" or self.to_web_Delivery_Status=="CANCELLED" or self.to_web_Delivery_Status=="INTERRUPTED":
                        update_deli_dt=requests.get(self.url + "/at80_web/real_app/api/agv_deliupdate.php?id={}&status={}".format(self.to_web_Delivery_ID, self.to_web_Delivery_Status))
...
if __name__=="__main__":
    rospy.init_node("web_server_handler_v8")
    WebServerHandlerV8()
    rospy.spin()
