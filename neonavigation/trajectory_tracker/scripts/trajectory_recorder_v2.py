    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            # Record the poses that dist_interval are more than the threshold value
                ...
                pathWV.poses.append(poseWV)
                self.pathWV_pub.publish(pathWV)
            # Record the poses that ang_interval are more than the threshold value
...
#        return yaw_difference
    ...
    def toPoseStampedWithVelocity(self, position, orientation, speed, seq):
        pos = Point(np.around(position.x, decimals=2), np.around(position.y, decimals=2), np.around(position.z, decimals=2))
        ...
        pose_WV.pose.position = pos
        pose_WV.pose.orientation = orientation
        pose_WV.linear_velocity = Vector3(np.around(speed, decimals=1),0,0)
        return pose_WV
    ...
    def saveTrajectory(self):
        with open(self.filepath, 'w') as outfile:
            if self.use_pickle:
                # Serializing python object structure using pickle
                pickle.dump(self.save_path_WV, outfile, 2)
            elif self.use_json:
                # Using json
                outfile.write(json.dumps(self.toJson(self.save_path_WV), indent=4, sort_keys=True))


if __name__=="__main__":
    rospy.init_node("trajectory_recorder_v2")
