...
protected:
  ...
  char motor_rpm_toss; //SAMUEL

  int32_t odom_encoder_left;
  int32_t odom_encoder_right;
  int32_t motor_rpm_left; //SAMUEL
  int32_t motor_rpm_right; //SAMUEL
...
MainNode::MainNode() : 
  ...
  motor_rpm_toss(5), //SAMUEL
  motor_rpm_left(0), //SAMUEL
  motor_rpm_right(0), //SAMUEL
  odom_x(0.0),
  odom_y(0.0),
  odom_yaw(0.0),
  odom_last_x(0.0),
  ...
  encoder_cpr(0)
{
  ...
  nhLocal.param("gear_ratio", gear_ratio, 1.0); //SAMUEL
  ROS_INFO_STREAM("gear_ratio: " << gear_ratio); //SAMUEL
...
void MainNode::cmdvel_callback( const geometry_msgs::Twist& twist_msg) {
  ...
  std::cout << "--------------- AGV' Speed ---------------" << std::endl; //SAMUEL
  std::cout << "x_speed: " << twist_msg.linear.x << std::endl; //SAMUEL
  std::cout << "z_speed: " << twist_msg.angular.z << std::endl; //SAMUEL
...
ROS_DEBUG_STREAM("cmdvel speed right: " << right_speed << " left: " << left_speed);
  ...
  std::cout << "--------------- Wheels' Speed ---------------" << std::endl; //SAMUEL
  std::cout << "R_speed: " << right_speed << std::endl; //SAMUEL
  std::cout << "L_speed: " << left_speed << std::endl; //SAMUEL
...
#endif
//    std::cout << "--------------- Open Loop ---------------" << std::endl; //SAMUEL
//    std::cout << "R_power: " << right_power << std::endl; //SAMUEL
//    std::cout << "L_power: " << left_power << std::endl; //SAMUEL
    ...
    // motor speed (rpm)
    int32_t right_rpm = right_speed / wheel_circumference * 60.0 * gear_ratio; //SAMUEL
    int32_t left_rpm = left_speed / wheel_circumference * 60.0 * gear_ratio; //SAMUEL
#ifdef _CMDVEL_DEBUG
ROS_DEBUG_STREAM("cmdvel rpm right: " << right_rpm << " left: " << left_rpm);
#endif
    std::cout << "--------------- RPM Command (Close Loop) ---------------" << std::endl; //SAMUEL
    std::cout << "R_rpm: " << right_rpm << std::endl; //SAMUEL
    std::cout << "L_rpm: " << left_rpm << std::endl; //SAMUEL

    std::cout << "--------------- Hall RPM ---------------" << std::endl; //SAMUEL
    std::cout << "R_rpm: " << motor_rpm_right << std::endl; //SAMUEL
    std::cout << "L_rpm: " << motor_rpm_left << std::endl; //SAMUEL

    std::cout << "--------------- Hall Relative Counter ---------------" << std::endl; //SAMUEL
    std::cout << "R_hall: " << odom_encoder_right << std::endl; //SAMUEL
    std::cout << "L_hall: " << odom_encoder_left << std::endl; //SAMUEL
...
//  controller.write("^RWD 250\r");
  ...
  controller.write("^ALIM 1 200\r");
  controller.write("^ALIM 2 200\r");

  // set max speed (3000 rpm) for relative speed commands
  controller.write("^MXRPM 1 3000\r");
  controller.write("^MXRPM 2 3000\r");

  // set max acceleration rate (1000 rpm/s * 10)
  ...
  // set PID parameters (gain * 1,000,000)
  controller.write("^KP 1 0\r");
  controller.write("^KP 2 0\r");
  controller.write("^KI 1 5000000\r");
  controller.write("^KI 2 5000000\r");
  controller.write("^KD 1 2000\r");
  controller.write("^KD 2 2000\r");
...
//  std::stringstream right_enccmd; //SAMUEL
//  std::stringstream left_enccmd; //SAMUEL
//  right_enccmd << "^EPPR 1 " << encoder_ppr << "\r"; //SAMUEL
//  left_enccmd << "^EPPR 2 " << encoder_ppr << "\r"; //SAMUEL
//  controller.write(right_enccmd.str()); //SAMUEL
//  controller.write(left_enccmd.str()); //SAMUEL
...
#ifdef _ODOM_SENSORS
/*
?CR  -->  Read Encoder Count Relative [Count]
?CB  -->  Read Absolute Brushless Counter [Count]
?BCR -->  Read Brushless Count Relative [Count]
?BS  -->  Read BL Motor Speed [RPM]
?BA  -->  Read Battery Amps [A]
?V   -->  Read Volts [V]
?T   -->  Read Temperature ['C]
*/
...
//  controller.write("# C_?BS_?BA_?V_# 11\r");
  controller.write("# C_?BCR_?BS_?BA_?V_?T_# 11\r");
#else
...
void MainNode::odom_loop() {
  ...
  if ( controller.available() ){
    char ch = 0;
    if ( controller.read((uint8_t*)&ch, 1) == 0 ) return;
    if (ch == '\r'){
      odom_buf[odom_idx] = 0;
#ifdef _ODOM_DEBUG
//ROS_DEBUG_STREAM( "line: " << odom_buf );
#endif
      // CR= is Encoder Count Relative [Count]
      if ( odom_buf[0] == 'C' && odom_buf[1] == 'R' && odom_buf[2] == '=' ) {
        for ( int delim = 3; delim < odom_idx; delim++ ) {
...
ROS_DEBUG_STREAM("encoder right: " << odom_encoder_right << " left: " << odom_encoder_left);
      ...
      // CB= is Absolute Brushless Counter [Count]
      else if ( odom_buf[0] == 'C' && odom_buf[1] == 'B' && odom_buf[2] == '=' ) {
        for ( int delim = 3; delim < odom_idx; delim++ ) {
          if ( odom_encoder_toss > 0 ) {
            --odom_encoder_toss;
            break;
      ...
      // BCR= is Brushless Count Relative [Count]
      else if ( odom_buf[0]=='B' && odom_buf[1]=='C' && odom_buf[2]=='R' && odom_buf[3]=='=') {
        for ( int delim = 3; delim < odom_idx; delim++ ) {
          if ( odom_encoder_toss > 0 ) {
            --odom_encoder_toss;
            break;
      ...
      // BS= is BL Motor Speed [RPM]
      else if ( odom_buf[0]=='B' && odom_buf[1]=='S' && odom_buf[2]=='=' ) {
        for ( int delim = 3; delim < odom_idx; delim++ ) {
          if ( motor_rpm_toss > 0) {
            --motor_rpm_toss;
            break;
          ...
          if ( odom_buf[delim] == ':') {
            odom_buf[delim] = 0;
            motor_rpm_right = (int32_t)strtol(odom_buf+3, NULL, 10);
            motor_rpm_left = (int32_t)strtol(odom_buf+delim+1, NULL, 10);
...
//ROS_DEBUG_STREAM("current right: " << current_right << " left: " << current_left);
      ...
      // T= is Temperature ['C]
      else if( odom_buf[0] =='T' && odom_buf[1] == '=') {
        for (int delim = 2; delim < odom_idx; delim++ ) {
          if (odom_buf[delim] == ':') {
            odom_buf[delim] = 0;
            temperature = (float)strtol(odom_buf+2, NULL, 10);
            break;
          }
        }
      }
...
//  float vyaw = (odom_yaw - odom_last_yaw) / dt;
  float vR = (motor_rpm_right*wheel_circumference)/(60.0*gear_ratio); //SAMUEL
  float vL = (motor_rpm_left*wheel_circumference)/(60.0*gear_ratio); //SAMUEL
  float vx = (vR + vL)/2.0; //SAMUEL
  float vy = 0; //SAMUEL
  float vyaw = 2.0*(vR - vL)/track_width; //SAMUEL
