...
class SnapshotV2():
    def __init__(self):
        ...
        # Service Server
        self.snapshot_srv = rospy.Service("/snapshot", Empty, self.snapshotSRV)

        # Service Client
        if self.report_to_web:
            self.file_call = rospy.ServiceProxy("/file_location/snapshot", SetFileLocation)
            self.error_call = rospy.ServiceProxy("/health/image", Empty)

    # snapshot Service
    def snapshotSRV(self, req):
