...
Features: (1) Added to rotate_HOME.path
    ...
    [New] (3) Added Manual triggering "Charging" from FMS
    [New] (4) Cap the maximum battery percent to 100% before sending to web_server_handler
"""
...
class NavigationMasterV1_8():
    ...
    def missionCB(self, msg):
        if(self.last_msg != msg.data and msg.data != "" and self.battery_safe and (self.delivery_status=="COMPLETED" or self.delivery_status=="" or self.delivery_status=="CANCELLED") and not self.charging_flag):
            self.resetVariables()
...
#        self.delivery_status = "CANCELLED"
    ...
    def charging_done_checker(self):
        ...
        if((self.battery_safe==False and (self.delivery_status=="COMPLETED" or self.delivery_status=="")) or (self.charging_flag and self.delivery_status=="DEPARTED")):
            time.sleep(2)
...
if __name__=="__main__":
    rospy.init_node("navigation_master_v1_8")
    NavigationMasterV1_8()
    rospy.spin()
