...
from atlas80evo_msgs.srv import SetFSMState, SetFileLocation, SetFileName
from trajectory_tracker_msgs.srv import ChangePath


class NavigationMasterV1_6():
    def __init__(self):
...
#        if(self.last_msg != msg.data and msg.data != "" and self.battery_safe and (self.delivery_status == "COMPLETED" or self.delivery_status == "")):
        if(self.last_msg != msg.data and msg.data != "" and self.battery_safe and (self.delivery_status=="COMPLETED" or self.delivery_status=="" or self.delivery_status=="CANCELLED")):
            self.resetVariables()
...
#        self.delivery_status = "CANCELLED"
        ...
        self.mission_activity = "NO ACTION"
#        if(self.fsm_state!="ESTOP" and self.fsm_state!="STANDBY"):
        if(self.fsm_state!="ESTOP" and self.fsm_state!="MANUAL" and self.fsm_state!="ERROR"):
    ...
    def route_doneSRV(self, req):
        if(self.route_once==False and self.delivery_status!="CHARGING"):
            self.route_seq += 1
            self.action_status = False
            self.route_once = True
            self.action_seq += 1
            self.delivery_status = "ARRIVED"
#            self.path_call(Header(0, rospy.Time.now(), "map"), "", 0)
        elif(self.delivery_status=="CHARGING"):
            if self.go_charge:
    ...
    def main_loop(self):
        while not rospy.is_shutdown():
            ...
            if(self.route_list != [] and self.action_status and self.delivery_status!="CHARGING"):
    ...
    def route_done_checker(self, route_list, route_seq):
        ...
        if(route_list[route_seq] in self.path_dict and (self.route_once or self.restore)):
            ...
            if(self.fsm_state=="DELIVERY"):
                ...
                if(self.fsm_state!="SUSPEND" and self.fsm_state!="MANUAL" and self.fsm_state!="ESTOP"):
    ...
    def charging_done_checker(self):
        ...
        if(self.battery_safe==False and (self.delivery_status=="COMPLETED" or self.delivery_status=="")):
            time.sleep(3)
            # Change FSM State into TRANSITION - only use for forward driving of charging
            if(self.fsm_state!="TRANSITION" and self.fsm_state!="SUSPEND" and self.fsm_state!="MANUAL" and self.fsm_state!="ESTOP"):
...
#            self.mission_activity = "CHARGE"
            self.battery_full = False
        # Charging
        elif(self.battery_safe==False and self.battery_full==False and self.delivery_status=="CHARGING" and self.start_charge):
            self.mission_activity = "CHARGE"
            self.charging_action()
            self.start_charge = False
            self.delivery_status = "CHARGING"
        # Navi back to Home
        elif(self.battery_safe and self.battery_full and self.delivery_status=="CHARGING" and self.go_home==False):
            # Change FSM State into TRANSITION - only use for forward driving of charging
            if(self.fsm_state!="TRANSITION" and self.fsm_state!="SUSPEND" and self.fsm_state!="MANUAL" and self.fsm_state!="ESTOP"):
                self.fsm_call("TRANSITION")
                self.go_home = True
...
if __name__=="__main__":
    rospy.init_node("navigation_master_v1_6")
    NavigationMasterV1_6()
    rospy.spin()
