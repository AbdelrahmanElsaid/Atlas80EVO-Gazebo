...
class LaneDepart(object):
    ...
    def __init__(self):
        ...
        #self.state_nerr=["SUSPEND","CHARGING","TABLE_PICKING","TABLE_DROPPING","MANUAL","STANDBY","NONE"]
    ...
    def recognition(self, color_image):
        obb = self.obb_cascade.detectMultiScale(color_image, 1.1, 5)
        ...
        if obb is not None:
            ...
            if not self.line_flag and (self.current_state=="DELIVERY" or self.current_state=="ERROR"):
            #if not self.line_flag and self.current_state not in self.state_nerr:
